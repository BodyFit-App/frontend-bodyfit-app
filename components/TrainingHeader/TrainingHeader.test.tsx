...
const snapshotFile = path.join(__dirname, '__snapshots__', 'TrainingHeader.test.tsx.snap');
...
describe("TrainingHeader Component", () => {
  ...
  it("renders training information correctly", () => {
    const { getByText, getByTestId } = render(<TrainingHeader {...defaultProps} />);
  ...
  it("calls onToggleFavorite when favorite button is pressed", () => {
    const { getByTestId } = render(<TrainingHeader {...defaultProps} isFavorite={true} />);
  ...
  it('renders the correct icon when isFavorite is true or false', () => {
    const { toJSON, rerender } = render(<TrainingHeader {...defaultProps} isFavorite={true} />);
